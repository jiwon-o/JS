...
// Promise is a JavaScript object for asynchronous operation.(프로미스는 비동기적 기능을 수행할 때 콜백함수 대신 유용하게 쓸 수 있는 JS 오브젝트)
...
// setTimeout()은 비동기적 함수기 때문에 콜백함수나 프로미스를 사용하지 않으면 기다리지 않고 넘어가서 undefined가 출력된다. 따라서 promise를 사용해야한다.
function print() {
  setTimeout(() => {
    return "jiwon";
  }, 2000);
...
// 1. Producer
// 새로운 Promise가 만들어질 때, 기본적으로 executor라는 콜백함수(resolve, reject)가 바로 실행이 된다.
...
fetchNumber
  ...
  .then((num) => {
    ...
    }); // then은 값을 바로 전달해도 되고 promise를 전달할 수도 있다.
  })
...
const getEgg = (hen) =>
  new Promise((resolve, reject) => {
    // setTimeout(() => resolve(`${hen} => 🥚`), 1000); // 성공했을 때
    setTimeout(() => reject(new Error(`error!`)), 1000); // 실패했을 때
  });
const cook = (egg) =>
  new Promise((resolve, reject) => {
